...
def run(local_rank, args):
    ...
    model.train(train_data, objective=objective, metric=metric, params=params)
    ...
    point_metric = model.metric(yhat_point, y_test)
    ...
    dist.all_reduce(point_metric, op=dist.ReduceOp.SUM)
    dist.all_reduce(crps, op=dist.ReduceOp.SUM)
    point_metric /= args.size
    crps /= args.size
    # Print final scores on rank 0 process.
    if local_rank == 0:
        print(f'Point metric PGBM: {point_metric:.2f}')
