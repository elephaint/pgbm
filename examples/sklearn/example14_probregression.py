...
gbr_ls = HistGradientBoostingRegressor(
    loss="squared_error", **common_params
)
...
for quantile in quantiles:
    gbr = HistGradientBoostingRegressor(
        loss="quantile", with_variance=False, quantile=quantile,
        **common_params
    )
...
# sample from.
#
# We can optimize these parameters in a CV procedure. We will evaluate the probabilistic
...
from pgbm.sklearn import make_probabilistic_scorer, crps_ensemble
...
crps_unoptimized = crps_ensemble(y_dist_test_unoptimized, y_test)
crps_optimized = crps_ensemble(y_dist_test_optimized, y_test)
